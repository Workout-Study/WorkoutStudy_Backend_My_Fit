...
class FitPenaltyFilterControllerTest {
    ...
    fun `filter fit penalty by fit group controller success test`() {
        ...
        whenever(readFitPenaltyUseCase.fitPenaltyFilterByFitGroupId(any<FitPenaltyFilterByFitGroupCommand>()))
            .thenReturn(fitPenaltyFilteredResponse)
        ...
        resultActions.andExpect(status().isOk())
            ...
            .andDo(
                document(
                    "fit-penalty-filter-by-fit-group",
                    pathParameters(
                        parameterWithName(GlobalURI.PATH_VARIABLE_FIT_GROUP_ID)
                            .description("penalty list를 조회할 fit group id"),
                    ),
                    queryParameters(
                        parameterWithName("startDate")
                            .description("조회할 fit penalty 발생 날짜 시작 일자 ( 값 전송하지 않을시 이번달 조회 )"),
